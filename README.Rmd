...
<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/clinTrialData)](https://CRAN.R-project.org/package=clinTrialData)
...
Data is stored in Parquet format and accessed through the [`connector`](https://github.com/NovoNordisk-OpenSource/connector) package, giving a consistent API regardless of which study you are working with.
...
**cdisc_pilot** — Standard CDISC Pilot 01 study (10 ADaM, 22 SDTM datasets). Available immediately after installation, no download needed.
...
Use `list_data_sources()` to see all locally available studies and `list_available_studies()` to see everything on GitHub Releases.
...
1. **Prepare your data** as Parquet files organised by domain (e.g. `adam/`, `sdtm/`):
...
2. **Upload to a GitHub Release** — open an issue on the [repository](https://github.com/Lovemore-Gakava/clinTrialData/issues) to request a release slot, then use the helper script:
...
3. **Users can inspect and access it immediately** — no CRAN submission required:
